...
`Settings` 可从所有界面通过左上角齿轮按钮打开，关闭后回到原界面。`previousScreen` 记录来源界面。
...
main.cpp
  ...
  ├── game_state.hpp/cpp  — 核心规则引擎：牌型识别、大小比较、AI、技能效果、积分计算
  ...
  ├── run_state.hpp/cpp   — Run 级状态：关卡数、已获得技能、装备槽、镜像快照、积分
...
- **计分规则**：玩家胜利时，遍历敌人剩余手牌，每张牌按斗地主点数（3→1分...大王→15分）累加 × 10
- **本局积分**（`m_roundScore`）：最近一局的得分
- **累计积分**（`m_totalScore`）：当前 Run 跨关卡累加，新 Run 归零
- **历史最高**（`m_highScore`）：持久化到 `highscore.txt`，启动时加载，Run 结束时比较更新
- **积分显示**：主菜单和角色选择显示"历史最高 N"；其他非战斗界面右上角显示"累计 N"；战斗界面右下角角色立绘上方显示"累计 N"
- **胜利积分动画**：玩家获胜 → 暗色遮罩 → 敌人手牌逐张翻牌 → 等待点击 → 金色 "+N" 飞向累计积分
...
`recordBombPlayed()` 在玩家或敌人打出炸弹/火箭时调用，`m_bombMarks` 累积。满 3 印记时自动从抽牌堆生成随机炸弹（4张同点数，不足时合成虚拟牌）加入手牌，印记归零。生成时触发 `m_bombGenFlash = 120` 帧红色斜条纹闪光。仅炸弹收藏家可见。
...
- **手牌数标签**：敌方红色数字（手牌中间），我方蓝色数字（手牌中间），均有白底黑边方形背景
- **设置按钮**：左上角齿轮图标（所有界面），hover 变青，点击打开设置弹窗
- **出牌区**：中央（敌人在上，玩家在下）
- **出牌/不出按钮**：手牌上方居中 (`h*0.56`)，PlayerTurn、MomentumPlay、SchedulePlay 时可见
- **玩家手牌**：底部 (`handCardY = h*0.66`)，竖直排列，扇形排布
- **玩家技能槽**：左下角 (`w*0.03`, `h*0.83`)，显示 `skill00/01/03.png` 全幅图片 + 技能名
- **炸弹印记**：右下角，黑底星标 + "炸弹印记 N/3" + 红色斜条纹闪光，仅炸弹收藏家可见
- **角色立绘**：对战界面右下角白底青蓝描边框，显示对战专用立绘 `char_0/1/_2.png`
- **掌控者调度立绘**：屏幕上方中央 `char_2.png`，调度完成后飞向右下角
- **累计积分**：战斗界面右下角角色立绘上方，白底黑边
...
| **调度飞行** | 调度完成后，掌控者立绘从上方中央 smoothstep 飞向右下角 (~0.4s) |
| **炸弹闪光** | 印记集满生成炸弹时，印记区红色 45° 斜条纹闪烁 2 秒，alpha 衰减 |
| **胜利积分动画** | 暗色遮罩 → 敌人手牌逐张翻牌（自适应卡数） → 点击 → 金色"+N"飞向累计积分 |

### 角色选择 UI

- 三张角色卡牌使用 `char0/1/2.png`（500×857）全幅显示，角色名在图片下方白色区域
- 悬停时上浮 + 1.10x 缩放，底部弹出白底黑边说明面板（仅被动描述）
- 齿轮按钮左上角

### 过渡界面 UI

- 左侧卡池：已获得技能 `skill00/01/03.png` 全幅图片 + 技能名，白底黑边背景
- 右侧装备槽：3 个槽位，同尺寸技能图片 + 黑色描边 + 技能名，白底黑边背景
- 拖拽装备：卡池 → 槽位（装备），槽位 → 卡池（卸下），槽位之间（交换）
- 双击快捷：双击卡池卡片→飞入首个空槽，双击已装备槽→飞回卡池

### 音频

- **背景音乐**：`resources/music/first.mp3`，`sf::Music`，循环播放，默认音量 100%。备选文件 `Start.mp3`、`second.mp3` 未使用
...
|----|------|--------|-------|
| 0 | 连击之势 | 敌人连续 2 回合不出牌时触发；选 1 张手牌免费打出 | `skill00.png` |
| 1 | 顺子大师 | 顺子最低长度-1（4 张即可出顺子） | `skill01.png` |
| 2 | 王牌意志 | 你的小王只能被大王压制（炸弹无效） | `skill03.png` |

全部 `SkillType::PASSIVE`。技能卡牌使用全幅图片替代代码绘制框架，带黑色描边。
...
| 技能卡牌 | 1050:1498 (7:10) | ~105×150~195×278 | `skill00/01/03.png` 全幅，pool/slot 尺寸统一 `h*0.20` |
...
| 资源 | 文件 | 用途 |
|------|------|------|
| 牌面 | `images/card/card0~53.png` | 54张扑克牌面 |
| 牌背 | `images/character-card/card00.png` | 对战牌背 |
| 角色选择 | `images/character-card/char0/1/2.png` | 角色选择界面 (500×857) |
| 对战立绘 | `images/character-card/char_0/1/_2.png` | 对战右下角 (512×1024) |
| 技能S01 | `images/character-card/skill00.png` | 连击之势 (1050×1498) |
| 技能S02 | `images/character-card/skill01.png` | 顺子大师 (1050×1498) |
| 技能S03 | `images/character-card/skill03.png` | 王牌意志 (1050×1498) |
...
| `src/renderer.cpp` | 全部绘制 + 点击检测（~2600 行，最大文件） |
| `src/game_state.cpp` | 核心规则、AI、技能效果（含调度/印记）、积分计算、回合流转 |
| `src/run_state.cpp` | Run 状态：关卡、技能、积分持久化 |
